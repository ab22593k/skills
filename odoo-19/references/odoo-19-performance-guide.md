...
def _compute_count(self):
    ...
    # _read_group returns a list of tuples: [(related_recordset, count), ...]
    counts_data = other_model._read_group(domain, ['related_id'], ['__count'])
    mapped_data = {}
    for related, count in counts_data:
        mapped_data[related.id] = count
    for record in self:
        record.count = mapped_data.get(record.id, 0)
```

> **Gotcha — Many2one groupby values are recordsets**: When grouping by a Many2one field, the group value is a **singleton recordset**, not an `(id, name)` tuple as in old `read_group()`. Use `.id` to extract the integer ID:
>
> ```python
> results = other_model._read_group([], ['many2one_field'], ['__count'])
> for m2o_recordset, count in results:
>     m2o_id = m2o_recordset.id  # not m2o_recordset[0] or m2o_recordset.0
> ```
