...
```xml
<!-- Step 1: Define module category (if not already in an installed module) -->
<record id="module_category_my_module" model="ir.module.category">
    <field name="name">My Module</field>
    <field name="description">My Module Application</field>
</record>

<!-- Step 2: Define privilege -->
...
<!-- Step 3: Define group with privilege -->
...
> **Gotcha**: For custom modules, define your `ir.module.category` record explicitly in your own data file. Referencing a `base.module_category_*` that doesn't exist in the installed modules will cause an XML parsing error during module loading.
...
> **Common pitfall — `user.id` vs `user.partner_id.id`**: When the record rule domain references a field that is a Many2one to `res.partner` (e.g. `instructor_id`), use `user.partner_id.id` not `user.id` — because `user` is a `res.users` record and its ID differs from the linked partner ID.
>
> ```xml
> <!-- WRONG: user.id is a res.users ID, not a partner ID -->
> <field name="domain_force">[('instructor_id', '=', user.id)]</field>
>
> <!-- CORRECT: user.partner_id.id resolves to the linked partner -->
> <field name="domain_force">[('instructor_id', '=', user.partner_id.id)]</field>
> ```
>
> > **Note**: Both `user.partner_id` (recordset) and `user.partner_id.id` (integer) work in domain expressions — Odoo converts recordsets to IDs internally. Prefer `.id` for explicitness.

> **Record rule `perm_*` fields**: These control which operations the rule applies to, NOT whether the operation is allowed. ACL gates operations first (additive grant), then record rules restrict which records. Setting `perm_write=True` on a rule when ACL denies write (`perm_write=0`) does NOT grant write — the ACL check fails before the rule is evaluated. Keep `perm_*` on rules aligned with the corresponding ACL entries to avoid confusion.
