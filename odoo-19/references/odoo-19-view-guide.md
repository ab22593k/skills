...
Decorations can be placed on individual `<field>` elements (colours just that column) or on the `<list>` root (highlights the entire row).

```xml
<!-- Row-level decoration -->
<list string="My Models" decoration-danger="state == 'cancelled'">
    <field name="name"/>
    <field name="state"/>
</list>

<!-- Column-level decoration -->
<list string="My Models">
...
> **Do not** use `today()`, `datetime.date.today()`, or `current_date()` — these are function calls not available in the evaluation context.
...
> **Odoo 19**: The `states` attribute on buttons is **deprecated**. Use `invisible` with attribute validation expressions instead.
